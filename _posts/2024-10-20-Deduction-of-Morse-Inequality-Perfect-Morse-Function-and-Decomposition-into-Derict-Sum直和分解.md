...
$$
\mathfrak{M}_t(f\vert _{M^a};R)-P_t(M^a; R) = (1+t)R_a(t)
$$

其中$$f\vert _{M^a}$$表示$$f$$在$$M^a$$上的限制。归纳的奠基是容易的，我们主要看如何从$$M^a$$上的等式到$$M^b$$上的等式。
...
$$
\begin{aligned}
    &\mathfrak{M}_t(f\vert _{M^b};R)-P_t(M^b; R)\\
    & = \bigl(\mathfrak{M}_t(f\vert _{M^a};R)-P_t(M^a; R)\bigr) + (1+t)R_C(t)\\
\end{aligned}
$$
